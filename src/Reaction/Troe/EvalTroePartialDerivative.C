...
void OptReaction::evalTroePartialDerivative()const noexcept
{
    ...
    for (size_t i = 0;i<Troe.size()-remain_Troe;i=i+4)
    {
...

        {
            ...
            __m256d MM = _mm256_blendv_pd(one, M, cmp);
    ...
    else if(remain_Troe==2)
    {
        ...
        __m128d expTTss = _mm_loadu_pd(&this->tmp_Exp[i+this->nSpecies+this->Troe.size()]);
        __m128d expTTs = _mm_loadu_pd(&this->tmp_Exp[i+this->nSpecies+this->Troe.size()*2]);
        ...
        {
            __m128d dKdT = _mm_blendv_pd(dK0dT, _mm_mul_pd(Pr,dKinfdT), cmp);
            ...
            __m128d K = _mm_blendv_pd(K0, Kinf, cmp);
            ...
            __m128d MM = _mm_blendv_pd(one128, M, cmp);
            dKfdT = _mm_fmadd_pd(_mm_mul_pd(_mm_mul_pd(K0,invOnePlusPr),dFdT),MM,dKfdT); 
            _mm_storeu_pd(&this->dKfdT_[j0],dKfdT);
        }
        {
            __m128d KK = _mm_blendv_pd(_mm_mul_pd(K0,invKinf), one128, cmp);
            __m128d N2 = _mm_blendv_pd(dFdPr,_mm_mul_pd(Pr,dFdPr),cmp);
            __m128d N1 = _mm_mul_pd(F,invOnePlusPr);
            N1 = _mm_blendv_pd(_mm_sub_pd(_mm_setzero_pd(),N1),N1,cmp);    
        ...
        }
        {
            ...
            __m128d KF = _mm_blendv_pd(N,_mm_mul_pd(N,M),cmp);
    ...
    else if(remain_Troe==3)
    {
        ...
        double tmpi0 = this->tmp_Exp[i+0+this->nSpecies+this->Troe.size()];
        double tmpi1 = this->tmp_Exp[i+1+this->nSpecies+this->Troe.size()];
        double tmpi2 = this->tmp_Exp[i+2+this->nSpecies+this->Troe.size()];
        __m256d expTTss = _mm256_setr_pd(tmpi0,tmpi1,tmpi2,1);
        tmpi0 = this->tmp_Exp[i+0+this->nSpecies+this->Troe.size()*2];
        tmpi1 = this->tmp_Exp[i+1+this->nSpecies+this->Troe.size()*2];
        tmpi2 = this->tmp_Exp[i+2+this->nSpecies+this->Troe.size()*2];
