...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->reversibleReaction22index[k];

 
        const unsigned int sl0 = lhsSpeciesIndex1D22RR[lhsIndex+0];
        const unsigned int sl1 = lhsSpeciesIndex1D22RR[lhsIndex+1];
