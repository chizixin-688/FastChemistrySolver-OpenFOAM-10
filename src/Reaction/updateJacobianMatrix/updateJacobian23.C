...
void  FastChemistry::OptReaction::JF23RR
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->reversibleReaction23index[k];
        ...
        if(i>=this->Ikf[2] && i <this->Ikf[6])
        { 
            const unsigned int m = i - this->Ikf[2];

            __m256d dKfdC = _mm256_set1_pd(this->dKfdC_[m]);
            __m256d CF_ = _mm256_set1_pd(CF);            
            __m256d CR_ = _mm256_set1_pd(CR);  
            double dKrdC = 0;
            dKrdC = (this->dKfdC_[m]*invKc);
            __m256d dKrdC_ = _mm256_set1_pd(dKrdC);

            for(unsigned int j = 0; j < AlignSpecies;j=j+4)
            {
                //WdMdC = dMdC*(dKfdC*CF-dKrdC*CR)
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[m*this->AlignSpecies+j]);
                __m256d WdMdC = _mm256_mul_pd(_mm256_mul_pd(dKrdC_,dMdC),CR_);
                WdMdC = _mm256_fmsub_pd(_mm256_mul_pd(dKfdC,dMdC),CF_,WdMdC);
...
void  FastChemistry::OptReaction::JF23IR
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->irreversibleReaction23index[k];
...
void  FastChemistry::OptReaction::JF23NER
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->nonEquilibriumReaction23index[k];
