...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        ...
        const double invKp = invNegGstdByRT[sr0]*ExpNegGbyRT[sl0];
        const double invKc = std::min(invKp,FastChemistry::invKcLimiter);
        const double sumVdBdT = (dBdT[sr0] ) - (dBdT[sl0] );
        const double dKcdTByKc = sumVdBdT;
...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        ...
        { 
            ...
            __m256d Wv = _mm256_set1_pd(this->dKfdC_[k]*CF);
...
)const noexcept
{
    ...
    for(std::size_t k=0; k<end; k++)
    {
        ...
        { 
            ...
            double W = this->dKfdC_[k]*CF - CR*this->dKfdC_[k+this->Itbr[4]];
            __m256d Wv = _mm256_set1_pd(W);
            for(unsigned int j = 0; j < this->AlignSpecies;j=j+4)
            {
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[k*this->AlignSpecies+j]);
...
void  FastChemistry::OptReaction::JF11M
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
...
{
    std::size_t end = this->thirdBodyEnhancedReaction11index.size();
    std::size_t lhsIndex = 0;
    std::size_t rhsIndex = 0;
    for(std::size_t i=0; i<end; i++)
    {
        const unsigned int k = this->thirdBodyEnhancedReaction11index[i];
        const unsigned int sl0 = this->lhsSpeciesIndex1D11TBE[lhsIndex];
        const unsigned int sr0 = this->rhsSpeciesIndex1D11TBE[rhsIndex];
        __m256d Wv = _mm256_set1_pd(this->dKfdC_[k-this->Ikf[2]]);
