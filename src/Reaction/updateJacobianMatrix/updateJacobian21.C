/*---------------------------------------------------------------------------*\
  Description
      Computing the molar concentration based jacobian matrix. The function 
      is used for two-one reaction, e.g. A+A=B. A+B=C
      
      RR:  reversible reaction
      IR:  irreversible reaction reverse rate constant is zero
      NER: non-equilibrium reaction, reverse rate constant is computed using
           Arrhenius form instead of equilibrium rate constant
  Author
      Zixin Chi <chizixin@buaa.edu.cn>
\*---------------------------------------------------------------------------*/

//=============================================================================//

//---------------------------------
// 1. FastChemistry headers
//---------------------------------
#include "OptReaction.H"

//=============================================================================//

/*void  FastChemistry::OptReaction::updateJacobian21
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
    const double* __restrict__ ExpNegGbyRT,
    const double* __restrict__ dBdT
)const noexcept
{

    std::size_t end = reaction21index.size();

    std::size_t lhsIndex = 0;

    std::size_t rhsIndex = 0;

    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->reaction21index[k];

 
        const unsigned int sl0 = lhsSpeciesIndex1D21[lhsIndex+0];
        const unsigned int sl1 = lhsSpeciesIndex1D21[lhsIndex+1];
        
        const unsigned int sr0 = rhsSpeciesIndex1D21[rhsIndex+0];

        double Kf = this->Kf_[i];
        double dKfdT = this->dKfdT_[i];
        double Kr = 0;
        double dKrdT = 0;
        double invKc = 0;

        if(this->isIrreversible[i]==0)
        {
            const double Kp = (ExpNegGbyRT[sr0])/(ExpNegGbyRT[sl0]*ExpNegGbyRT[sl1]);
            double Kc = Kp*this->Pow_pByRT_SumVki[1];  
            Kc = Kc > KcLimiter?Kc:KcLimiter;
            const double sumVdBdT = (dBdT[sr0] ) - (dBdT[sl0] + dBdT[sl1]);
            const double dKcdTByKc = sumVdBdT +1*invT;
            invKc = 1.0/Kc;  
            Kr = Kf*invKc;        
            dKrdT = (dKfdT*invKc - (Kc > KcLimiter ? Kr*dKcdTByKc : 0));  
            
            const double dCrdC0 = 1;
            ddNdtByVdcTp[sl0*(this->alignN)+sr0] -= (-Kr*dCrdC0);
            ddNdtByVdcTp[sl1*(this->alignN)+sr0] -= (-Kr*dCrdC0);
            ddNdtByVdcTp[sr0*(this->alignN)+sr0] += (-Kr*dCrdC0);     
        }
        else if(this->isIrreversible[i]==2)
        {
            Kr = this->Kf_[i - Ikf[1] + Ikf[9]];
            dKrdT = this->dKfdT_[i - Ikf[1] + Ikf[9]];
            invKc = Kr/Kf;
            const double dCrdC0 = 1;
            ddNdtByVdcTp[sl0*(this->alignN)+sr0] -= (-Kr*dCrdC0);
            ddNdtByVdcTp[sl1*(this->alignN)+sr0] -= (-Kr*dCrdC0);
            ddNdtByVdcTp[sr0*(this->alignN)+sr0] += (-Kr*dCrdC0);
        }

        const double CF = C[sl0]*C[sl1];
        const double CR = C[sr0];

        const double q = (Kf*CF) - (Kr*CR);
        dNdtByV[sl0] = dNdtByV[sl0] - q; 
        dNdtByV[sl1] = dNdtByV[sl1] - q;
        dNdtByV[sr0] = dNdtByV[sr0] + q;

        const double dqdT = (dKfdT*CF)-(dKrdT*CR);
        ddNdtByVdcTp[sl0*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sl1*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sr0*(this->alignN)+(this->nSpecies)] += dqdT;
        
        const double dCfdC0 = C[sl1];
        const double dCfdC1 = C[sl0];
        ddNdtByVdcTp[sl0*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sl1*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sr0*(this->alignN)+sl0] += Kf*dCfdC0;
        ddNdtByVdcTp[sl0*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sl1*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sr0*(this->alignN)+sl1] += Kf*dCfdC1;

        if(i>=this->Ikf[2] && i <this->Ikf[6])
        { 
            const unsigned int k = i - this->Ikf[2];

            __m256d dKfdC = _mm256_set1_pd(this->dKfdC_[k]);
            __m256d CF_ = _mm256_set1_pd(CF);            
            __m256d CR_ = _mm256_set1_pd(CR);  
            double dKrdC = 0;


            dKrdC = (this->dKfdC_[k]*invKc);
            __m256d dKrdC_ = _mm256_set1_pd(dKrdC);

            for(unsigned int j = 0; j < this->AlignSpecies;j=j+4)
            {
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[k*this->AlignSpecies+j]);

                __m256d WdMdC = _mm256_mul_pd(_mm256_mul_pd(dKrdC_,dMdC),CR_);
                WdMdC = _mm256_fmsub_pd(_mm256_mul_pd(dKfdC,dMdC),CF_,WdMdC);


                __m256d sl0v = load256d(&ddNdtByVdcTp[sl0*(this->alignN)+j]);
                sl0v = _mm256_sub_pd(sl0v,WdMdC);
                store256d(&ddNdtByVdcTp[sl0*(this->alignN)+j],sl0v);

                __m256d sl1v = load256d(&ddNdtByVdcTp[sl1*(this->alignN)+j]);
                sl1v = _mm256_sub_pd(sl1v,WdMdC);
                store256d(&ddNdtByVdcTp[sl1*(this->alignN)+j],sl1v);

                __m256d sr0v = load256d(&ddNdtByVdcTp[sr0*(this->alignN)+j]);
                sr0v = _mm256_add_pd(sr0v,WdMdC);
                store256d(&ddNdtByVdcTp[sr0*(this->alignN)+j],sr0v);                              
            } 
        }    


        lhsIndex = lhsIndex + 2;
        rhsIndex = rhsIndex + 1;
    }
}   */


void  FastChemistry::OptReaction::JF21RR
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
    const double* __restrict__ ExpNegGbyRT,
    const double* __restrict__ dBdT
)const noexcept
{

    std::size_t end = reversibleReaction21index.size();

    std::size_t lhsIndex = 0;

    std::size_t rhsIndex = 0;

    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->reversibleReaction21index[k];

 
        const unsigned int sl0 = lhsSpeciesIndex1D21RR[lhsIndex+0];
        const unsigned int sl1 = lhsSpeciesIndex1D21RR[lhsIndex+1];
        
        const unsigned int sr0 = rhsSpeciesIndex1D21RR[rhsIndex+0];

        double Kf = this->Kf_[i];
        double dKfdT = this->dKfdT_[i];



                

        /*//const double Kp333 = (ExpNegGbyRT[sr0])*(invNegGstdByRT[sl0]*invNegGstdByRT[sl1]);
        const double Kp = (ExpNegGbyRT[sr0])/(ExpNegGbyRT[sl0]*ExpNegGbyRT[sl1]);
        //std::cout<<Kp<<" "<<Kp333<<" "<<(Kp-Kp333)/Kp333<<std::endl;
        double Kc = Kp*this->Pow_pByRT_SumVki[1];
        Kc = Kc > KcLimiter?Kc:KcLimiter;
        const double sumVdBdT = (dBdT[sr0] ) - (dBdT[sl0] + dBdT[sl1]);
        const double dKcdTByKc = sumVdBdT +1*invT;
        invKc = 1.0/Kc;  
        Kr = Kf*invKc;        
        dKrdT = (dKfdT*invKc - (Kc > KcLimiter ? Kr*dKcdTByKc : 0));*/

        const double invKp = (invNegGstdByRT[sr0])*(ExpNegGbyRT[sl0]*ExpNegGbyRT[sl1]);
        double invKc = invKp*this->Pow_pByRT_SumVki[3];  
        invKc = std::min(invKc,FastChemistry::invKcLimiter);
        const double sumVdBdT = (dBdT[sr0] ) - (dBdT[sl0] + dBdT[sl1]);
        const double dKcdTByKc = sumVdBdT +1*invT;
        const double Kr = Kf*invKc;        
        const double dKrdT = (dKfdT*invKc - (invKc < FastChemistry::invKcLimiter ? Kr*dKcdTByKc : 0));


            
        const double dCrdC0 = 1;
        ddNdtByVdcTp[sl0*(this->alignN)+sr0] -= (-Kr*dCrdC0);
        ddNdtByVdcTp[sl1*(this->alignN)+sr0] -= (-Kr*dCrdC0);
        ddNdtByVdcTp[sr0*(this->alignN)+sr0] += (-Kr*dCrdC0);     
        


        const double CF = C[sl0]*C[sl1];
        const double CR = C[sr0];

        const double q = (Kf*CF) - (Kr*CR);
        dNdtByV[sl0] = dNdtByV[sl0] - q; 
        dNdtByV[sl1] = dNdtByV[sl1] - q;
        dNdtByV[sr0] = dNdtByV[sr0] + q;

        const double dqdT = (dKfdT*CF)-(dKrdT*CR);
        ddNdtByVdcTp[sl0*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sl1*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sr0*(this->alignN)+(this->nSpecies)] += dqdT;
        
        const double dCfdC0 = C[sl1];
        const double dCfdC1 = C[sl0];
        ddNdtByVdcTp[sl0*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sl1*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sr0*(this->alignN)+sl0] += Kf*dCfdC0;
        ddNdtByVdcTp[sl0*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sl1*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sr0*(this->alignN)+sl1] += Kf*dCfdC1;

        if(i>=this->Ikf[3] && i <this->Ikf[6])
        { 
            const unsigned int k = i - this->Ikf[2];


            double W = this->dKfdC_[k]*(CF-invKc*CR);

            //this->dKfdC_[k] = W;
            __m256d Wv = _mm256_set1_pd(W);
            for(unsigned int j = 0; j < this->AlignSpecies;j=j+4)
            {
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[k*this->AlignSpecies+j]);

                __m256d WdMdC = _mm256_mul_pd(Wv,dMdC);


                __m256d sl0v = load256d(&ddNdtByVdcTp[sl0*(this->alignN)+j]);
                sl0v = _mm256_sub_pd(sl0v,WdMdC);
                store256d(&ddNdtByVdcTp[sl0*(this->alignN)+j],sl0v);

                __m256d sl1v = load256d(&ddNdtByVdcTp[sl1*(this->alignN)+j]);
                sl1v = _mm256_sub_pd(sl1v,WdMdC);
                store256d(&ddNdtByVdcTp[sl1*(this->alignN)+j],sl1v);

                __m256d sr0v = load256d(&ddNdtByVdcTp[sr0*(this->alignN)+j]);
                sr0v = _mm256_add_pd(sr0v,WdMdC);
                store256d(&ddNdtByVdcTp[sr0*(this->alignN)+j],sr0v);                              
            } 
        }    


        lhsIndex = lhsIndex + 2;
        rhsIndex = rhsIndex + 1;
    }
}   


void  FastChemistry::OptReaction::JF21IR
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
    const double* __restrict__ ExpNegGbyRT,
    const double* __restrict__ dBdT
)const noexcept
{

    std::size_t end = irreversibleReaction21index.size();

    std::size_t lhsIndex = 0;

    std::size_t rhsIndex = 0;

    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->irreversibleReaction21index[k];

 
        const unsigned int sl0 = lhsSpeciesIndex1D21IR[lhsIndex+0];
        const unsigned int sl1 = lhsSpeciesIndex1D21IR[lhsIndex+1];
        
        const unsigned int sr0 = rhsSpeciesIndex1D21IR[rhsIndex+0];

        double Kf = this->Kf_[i];
        double dKfdT = this->dKfdT_[i];


        const double CF = C[sl0]*C[sl1];

        const double q = (Kf*CF);
        dNdtByV[sl0] = dNdtByV[sl0] - q; 
        dNdtByV[sl1] = dNdtByV[sl1] - q;
        dNdtByV[sr0] = dNdtByV[sr0] + q;

        const double dqdT = (dKfdT*CF);
        ddNdtByVdcTp[sl0*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sl1*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sr0*(this->alignN)+(this->nSpecies)] += dqdT;
        
        const double dCfdC0 = C[sl1];
        const double dCfdC1 = C[sl0];
        ddNdtByVdcTp[sl0*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sl1*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sr0*(this->alignN)+sl0] += Kf*dCfdC0;
        ddNdtByVdcTp[sl0*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sl1*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sr0*(this->alignN)+sl1] += Kf*dCfdC1;

        if(i>=this->Ikf[3] && i <this->Ikf[6])
        { 
            const unsigned int k = i - this->Ikf[2];

            double W = this->dKfdC_[k]*CF;

            //this->dKfdC_[k] = W;
            __m256d Wv = _mm256_set1_pd(W);
            for(unsigned int j = 0; j < this->AlignSpecies;j=j+4)
            {
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[k*this->AlignSpecies+j]);


                __m256d WdMdC = _mm256_mul_pd(Wv,dMdC);

                __m256d sl0v = load256d(&ddNdtByVdcTp[sl0*(this->alignN)+j]);
                sl0v = _mm256_sub_pd(sl0v,WdMdC);
                store256d(&ddNdtByVdcTp[sl0*(this->alignN)+j],sl0v);

                __m256d sl1v = load256d(&ddNdtByVdcTp[sl1*(this->alignN)+j]);
                sl1v = _mm256_sub_pd(sl1v,WdMdC);
                store256d(&ddNdtByVdcTp[sl1*(this->alignN)+j],sl1v);

                __m256d sr0v = load256d(&ddNdtByVdcTp[sr0*(this->alignN)+j]);
                sr0v = _mm256_add_pd(sr0v,WdMdC);
                store256d(&ddNdtByVdcTp[sr0*(this->alignN)+j],sr0v);                              
            }
        }    


        lhsIndex = lhsIndex + 2;
        rhsIndex = rhsIndex + 1;
    }
}   


void  FastChemistry::OptReaction::JF21NER
(
    const double* __restrict__ C,
    double* __restrict__ dNdtByV,
    double* __restrict__ ddNdtByVdcTp,
    const double* __restrict__ ExpNegGbyRT,
    const double* __restrict__ dBdT
)const noexcept
{

    std::size_t end = nonEquilibriumReaction21index.size();

    std::size_t lhsIndex = 0;

    std::size_t rhsIndex = 0;

    for(std::size_t k=0; k<end; k++)
    {
        const unsigned int i = this->nonEquilibriumReaction21index[k];

 
        const unsigned int sl0 = lhsSpeciesIndex1D21NER[lhsIndex+0];
        const unsigned int sl1 = lhsSpeciesIndex1D21NER[lhsIndex+1];
        
        const unsigned int sr0 = rhsSpeciesIndex1D21NER[rhsIndex+0];

        double Kf = this->Kf_[i];
        double dKfdT = this->dKfdT_[i];
        double Kr = 0;
        double dKrdT = 0;
        //double invKc = 0;



        
        Kr = this->Kf_[i - Ikf[1] + Ikf[9]];
        dKrdT = this->dKfdT_[i - Ikf[1] + Ikf[9]];
        //invKc = Kr/Kf;
        const double dCrdC0 = 1;
        ddNdtByVdcTp[sl0*(this->alignN)+sr0] -= (-Kr*dCrdC0);
        ddNdtByVdcTp[sl1*(this->alignN)+sr0] -= (-Kr*dCrdC0);
        ddNdtByVdcTp[sr0*(this->alignN)+sr0] += (-Kr*dCrdC0);
        

        const double CF = C[sl0]*C[sl1];
        const double CR = C[sr0];

        const double q = (Kf*CF) - (Kr*CR);
        dNdtByV[sl0] = dNdtByV[sl0] - q; 
        dNdtByV[sl1] = dNdtByV[sl1] - q;
        dNdtByV[sr0] = dNdtByV[sr0] + q;

        const double dqdT = (dKfdT*CF)-(dKrdT*CR);
        ddNdtByVdcTp[sl0*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sl1*(this->alignN)+(this->nSpecies)] -= dqdT;
        ddNdtByVdcTp[sr0*(this->alignN)+(this->nSpecies)] += dqdT;
        
        const double dCfdC0 = C[sl1];
        const double dCfdC1 = C[sl0];
        ddNdtByVdcTp[sl0*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sl1*(this->alignN)+sl0] -= Kf*dCfdC0;
        ddNdtByVdcTp[sr0*(this->alignN)+sl0] += Kf*dCfdC0;
        ddNdtByVdcTp[sl0*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sl1*(this->alignN)+sl1] -= Kf*dCfdC1;
        ddNdtByVdcTp[sr0*(this->alignN)+sl1] += Kf*dCfdC1;

        if(i>=this->Ikf[2] && i <this->Ikf[3])
        { 
            const unsigned int k = i - this->Ikf[2];


            double W = this->dKfdC_[k]*CF - this->dKfdC_[k+this->Itbr[4]]*CR;

            //this->dKfdC_[k] = W;
            __m256d Wv = _mm256_set1_pd(W);
            for(unsigned int j = 0; j < this->AlignSpecies;j=j+4)
            {
                __m256d dMdC = load256d(&this->ThirdBodyFactor1D[k*this->AlignSpecies+j]);

                __m256d WdMdC = _mm256_mul_pd(Wv,dMdC);


                __m256d sl0v = load256d(&ddNdtByVdcTp[sl0*(this->alignN)+j]);
                sl0v = _mm256_sub_pd(sl0v,WdMdC);
                store256d(&ddNdtByVdcTp[sl0*(this->alignN)+j],sl0v);

                __m256d sl1v = load256d(&ddNdtByVdcTp[sl1*(this->alignN)+j]);
                sl1v = _mm256_sub_pd(sl1v,WdMdC);
                store256d(&ddNdtByVdcTp[sl1*(this->alignN)+j],sl1v);

                __m256d sr0v = load256d(&ddNdtByVdcTp[sr0*(this->alignN)+j]);
                sr0v = _mm256_add_pd(sr0v,WdMdC);
                store256d(&ddNdtByVdcTp[sr0*(this->alignN)+j],sr0v);                              
            } 
        }    


        lhsIndex = lhsIndex + 2;
        rhsIndex = rhsIndex + 1;
    }
}   


void  FastChemistry::OptReaction::JF21M
(
    double* ddNdtByVdcTp
)const noexcept
{
    std::size_t end = this->thirdBodyEnhancedReaction21index.size();
    std::size_t remain = end%4;
    std::size_t lhsIndex = 0;

    std::size_t rhsIndex = 0;
    if(this->AlignSpecies%8==0)
    {
        for(std::size_t k=0; k<end; k=k+1)
        {
            const unsigned int i0 = this->thirdBodyEnhancedReaction21index[k+0];


            const unsigned int m0 = i0 - this->Ikf[2];
            __m256d Wv0 = _mm256_set1_pd(this->dKfdC_[m0]);

            const unsigned int sl0a = this->lhsSpeciesIndex1D21TBE[lhsIndex+0];
            const unsigned int sl1a = this->lhsSpeciesIndex1D21TBE[lhsIndex+1];

            const unsigned int sr0a = this->rhsSpeciesIndex1D21TBE[rhsIndex+0];

            for(unsigned int j = 0; j < this->AlignSpecies;j=j+8)
            {
                __m256d dM0dC03v = load256d(&this->ThirdBodyFactor1D[m0*this->AlignSpecies+j+0]);
                __m256d dM0dC47v = load256d(&this->ThirdBodyFactor1D[m0*this->AlignSpecies+j+4]);

                __m256d WdM0dC03v = _mm256_mul_pd(Wv0,dM0dC03v);
                __m256d WdM0dC47v = _mm256_mul_pd(Wv0,dM0dC47v);
                {
                __m256d sl0va = load256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0]);
                sl0va = _mm256_sub_pd(sl0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0],sl0va);
                }
                {
                __m256d sl0va = load256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+4]);
                sl0va = _mm256_sub_pd(sl0va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+4],sl0va);
                }

                {
                __m256d sl1va = load256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0]);
                sl1va = _mm256_sub_pd(sl1va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0],sl1va);
                }
                {
                __m256d sl1va = load256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+4]);
                sl1va = _mm256_sub_pd(sl1va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+4],sl1va);
                }

                {
                __m256d sr0va = load256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0]);
                sr0va = _mm256_add_pd(sr0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0],sr0va);
                }
                {
                __m256d sr0va = load256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+4]);
                sr0va = _mm256_add_pd(sr0va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+4],sr0va);
                }
            } 
            lhsIndex = lhsIndex + 2;
            rhsIndex = rhsIndex + 1;
        }
    }
    else if(this->AlignSpecies%8==4)
    {
        for(std::size_t k=0; k<end; k=k+1)
        {
            const unsigned int i0 = this->thirdBodyEnhancedReaction21index[k+0];


            const unsigned int m0 = i0 - this->Ikf[2];
            __m256d Wv0 = _mm256_set1_pd(this->dKfdC_[m0]);

            const unsigned int sl0a = lhsSpeciesIndex1D21TBE[lhsIndex+0];
            const unsigned int sl1a = lhsSpeciesIndex1D21TBE[lhsIndex+1];
            const unsigned int sr0a = rhsSpeciesIndex1D21TBE[rhsIndex+0];

            for(unsigned int j = 0; j < this->AlignSpecies-4;j=j+8)
            {
                __m256d dM0dC03v = load256d(&this->ThirdBodyFactor1D[m0*this->AlignSpecies+j+0]);
                __m256d dM0dC47v = load256d(&this->ThirdBodyFactor1D[m0*this->AlignSpecies+j+4]);

                __m256d WdM0dC03v = _mm256_mul_pd(Wv0,dM0dC03v);
                __m256d WdM0dC47v = _mm256_mul_pd(Wv0,dM0dC47v);
                {
                __m256d sl0va = load256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0]);
                sl0va = _mm256_sub_pd(sl0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0],sl0va);
                }
                {
                __m256d sl0va = load256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+4]);
                sl0va = _mm256_sub_pd(sl0va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+4],sl0va);
                }

                {
                __m256d sl1va = load256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0]);
                sl1va = _mm256_sub_pd(sl1va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0],sl1va);
                }
                {
                __m256d sl1va = load256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+4]);
                sl1va = _mm256_sub_pd(sl1va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+4],sl1va);
                }

                {
                __m256d sr0va = load256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0]);
                sr0va = _mm256_add_pd(sr0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0],sr0va);
                }
                {
                __m256d sr0va = load256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+4]);
                sr0va = _mm256_add_pd(sr0va,WdM0dC47v);
                store256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+4],sr0va);
                }
            }
            {
                unsigned int j = this->AlignSpecies-4;
                __m256d dM0dC03v = load256d(&this->ThirdBodyFactor1D[m0*this->AlignSpecies+j+0]);


                __m256d WdM0dC03v = _mm256_mul_pd(Wv0,dM0dC03v);

                {
                __m256d sl0va = load256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0]);
                sl0va = _mm256_sub_pd(sl0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl0a*(this->alignN)+j+0],sl0va);
                }


                {
                __m256d sl1va = load256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0]);
                sl1va = _mm256_sub_pd(sl1va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sl1a*(this->alignN)+j+0],sl1va);
                }


                {
                __m256d sr0va = load256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0]);
                sr0va = _mm256_add_pd(sr0va,WdM0dC03v);
                store256d(&ddNdtByVdcTp[sr0a*(this->alignN)+j+0],sr0va);
                }

            }
            lhsIndex = lhsIndex + 2;
            rhsIndex = rhsIndex + 1;
        }
    }
}
