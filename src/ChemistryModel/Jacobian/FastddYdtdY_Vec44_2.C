...
void Foam::FastChemistryModel<ThermoType>::FastddYdtdY_Vec44_2
(
...
{ 
    ...
    for(int i=0; i<this->nSpecie()-2; i=i+4)
    {
        ...
        for (int j=0; j<this->nSpecie()-2; j=j+4)
        {
        ...
        }
        {
            ...
            __m128d invWv = load128d(&invW[j+0]);//10
            __m128d dCjdYj = _mm_mul_pd(_mm256_castpd256_pd128(rhoMv),invWv);//10
            __m128d rhoMvj_ = load128d(&rhoMByRhoi[j+0]);//11

            __m128d Jci0j03v = load128d(&ddNdtByVdcT[(i+0)*(alignN)+j+0]);//12
            __m128d ddNi0dtByVdYj = _mm_mul_pd(Jci0j03v,dCjdYj);//12
            __m128d r0 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi0dtv));//13
            __m128d ddYi0dtdYj = _mm_fmadd_pd(_mm256_castpd256_pd128(Wi0ByrhoMv),ddNi0dtByVdYj,r0);//12
            store128d(&Jac[(i+0)*(alignN) + j+0],ddYi0dtdYj);//11

            __m128d Jci1j03v = load128d(&ddNdtByVdcT[(i+1)*(alignN)+j+0]);
            __m128d ddNi1dtByVdYj = _mm_mul_pd(Jci1j03v,dCjdYj);
            __m128d r1 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi1dtv));//13
            __m128d ddYi1dtdYj = _mm_fmadd_pd(_mm256_castpd256_pd128(Wi1ByrhoMv),ddNi1dtByVdYj,r1);
            store128d(&Jac[(i+1)*(alignN) + j+0],ddYi1dtdYj);

            __m128d Jci2j03v = load128d(&ddNdtByVdcT[(i+2)*(alignN)+j+0]);
            __m128d ddNi2dtByVdYj = _mm_mul_pd(Jci2j03v,dCjdYj);
            __m128d r2 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi2dtv));//13
            __m128d ddYi2dtdYj = _mm_fmadd_pd(_mm256_castpd256_pd128(Wi2ByrhoMv),ddNi2dtByVdYj,r2);
            store128d(&Jac[(i+2)*(alignN) + j+0],ddYi2dtdYj);

            __m128d Jci3j03v = load128d(&ddNdtByVdcT[(i+3)*(alignN)+j+0]);
            __m128d ddNi3dtByVdYj = _mm_mul_pd(Jci3j03v,dCjdYj);//12
            __m128d r3 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi3dtv));//13
            __m128d ddYi3dtdYj = _mm_fmadd_pd(_mm256_castpd256_pd128(Wi3ByrhoMv),ddNi3dtByVdYj,r3);
            store128d(&Jac[(i+3)*(alignN) + j+0],ddYi3dtdYj);
        }
    }
    {
