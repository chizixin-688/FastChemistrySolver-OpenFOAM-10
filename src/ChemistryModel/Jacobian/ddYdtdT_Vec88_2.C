...
void Foam::FastChemistryModel<UnusedThermo>::ddYdtdT_Vec88_2
(
...
) const noexcept
{
    ...
    {
        int i0 = this->nSpecie()-2;
        int i1 = this->nSpecie()-1;


        const double Wi0ByrhoM = WiByrhoM[i0];
        const double Wi1ByrhoM_ = WiByrhoM[i1];

        dPhidt[i0] = dPhidt[i0]*Wi0ByrhoM;
        ...
        Jac[i0*(alignN) + this->nSpecie()] = Wi0ByrhoM*ddNi0dtByVdT + alphavM*dYi0dt;
        Jac[i1*(alignN) + this->nSpecie()] = Wi1ByrhoM_*ddNi1dtByVdT + alphavM*dYi1dt;
    }
}