...
void Foam::FastChemistryModel<UnusedThermo>::ddYdtdY_Vec844_5
(
...
) const noexcept
{
    ...
    for(int j = 0; j < this->nSpecie()-5; j = j + 8)
    {
        ...
        // Do the last remain 5 species
        {
            ...
            const double rhoMByWiYTi5 = -rhoM*invW[i+4]*Phi[i+4];
    ...
    // Do remain 4 species, 1 species left
    {
        ...
        // Do the last remain 5 species
        {
            ...
            int ii = this->nSpecie()-1;
            const double rhoMByWiYTPi0 = -rhoM*invW[ii+0]*Phi[ii+0];
            __m256d r1 = _mm256_mul_pd(_mm256_set1_pd(rhoMByWiYTPi0),rhoMbyRhoJ03);
            store256d(&dcdY[i*4+16],r1);
        }
