...
void Foam::FastChemistryModel<UnusedThermo>::ddYdtdY_Vec844_4
(
...
) const noexcept
{
    ...
    for(int j = 0; j < this->nSpecie()-4; j = j + 8)
    {
        ...
        // Do the last remain 4 species
        {
            int i = this->nSpecie()-4;
            __m256d rhoMByWi03YTv = _mm256_mul_pd(load256d(&invW[i+0]), load256d(&Phi[i+0]));
            rhoMByWi03YTv = _mm256_mul_pd(-rhoMv,rhoMByWi03YTv);
