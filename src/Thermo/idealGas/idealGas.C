...
)const 
{
    ...
    if(remain==1)
    {
        ...
        dCpdT[i] = ( a[1] + T_*(2*a[2]+T_*(3*a[3]+T_*4*a[4])) )*this->invWRu[i]; 
        Cp[i] = ((((a[4]*T_+a[3])*T_+a[2])*T_+a[1])*T_+a[0])*this->invWRu[i];
        Ha[i] = (((((a[4]*T_*0.2+a[3]*0.25)*T_+a[2]*(1.0/3.0))*T_+a[1]*0.5)*T_+a[0])*T_ +a[5])*this->invWRu[i]; 
    }
    else if(remain==2)
    {
        ...
        this->rhoM += Phi[i+0]*invW[i+0];
        this->rhoM += Phi[i+1]*invW[i+1];
        ...
        __m128d RuInvW = load128d(&this->invWRu[i]);
    ...
    else if(remain==3)
    {
        ...
        //__m256d RuInvW = _mm256_mul_pd(invWv,Ruv);
        __m256d RuInvW = load256d(&this->invWRu[i]);
        __m256d vdCpdT = _mm256_fmadd_pd(vT,4*A4,3*A3);
        vdCpdT = _mm256_fmadd_pd(vT,vdCpdT,2*A2);  
        vdCpdT = _mm256_fmadd_pd(vT,vdCpdT,A1);  
